...
exports.createColaborador = async (req, res, next) => {
  ...
  Colaborador.create(req.body)
    .then(async (data) => {
      ...
      const from =
        '"Se agregó como un nuevo colaborador" <dgadeaio4@gmail.com>';
...
exports.findAllColaboradores = async (req, res, next) => {
  Colaborador.findAll({
    ...
    .catch((err) => {
      res.status(500).send({
        message:
          err.message || "Ocurrió un error al obtener los colaboradores.",
      });
    });
};
...
exports.findAllColaboradoresWithUser = async (req, res, next) => {
  try {
    ...
    const colaboradoresConUsuario = await Promise.all(
      colaboradores.map(async (colaborador) => {
        if (colaborador.idColaborador !== undefined) {
          // Verificar si el ID del colaborador está definido
        ...
        colaborador
          ...
          .catch((err) => {
            res.status(500).send({
              message:
                err.message ||
                `Ocurrió un error al eliminar el colaborador con ID ${id}`,
            });
            //next();
          });
      }
...
exports.findColaboradoresSinUsuario = async (req, res) => {
  try {
    ...
    const idsColaboradoresConUsuario = usuarios.map(
      (usuario) => usuario.idColaborador
    );

    // Filtrar colaboradores que no tienen usuario asignado
    const colaboradoresSinUsuario = colaboradores.filter(
      (colaborador) =>
        !idsColaboradoresConUsuario.includes(colaborador.idColaborador)
    );
  ...
  try {
    const nombresCumpleanierosHoy = (colaboradores) => {
      ...
      return colaboradores
        .filter((colaborador) => {
          ...
          return (
            diaNacimiento === hoy.getDate() && mesNacimiento === hoy.getMonth()
          );
        })
    ...
    const enviarCorreoCumpleanieros = async () => {
      ...
      const listaCorreos = colaboradores.map((colaborador) => ({
        // recuperamos todos los colaboradores y buscamos que se envien los mensajes
        ...
        mensaje: nombresCumpleanieros.includes(colaborador.nombre) // seleccion de mensaje a mostrar segun si esta de cumpleaños o no
          ...
          : `<div style="background-image: url('https://i.pinimg.com/564x/20/c0/dc/20c0dc691b73663021e7c6642abad5bc.jpg'); background-size: cover; color: #001F3F; width: 480px; height: 800px; text-align: center;">
                        <div style="margin: 0 auto; max-width: 80%; padding-top: 145px;">
                            <h2 style="font: oblique bold 120% cursive; font-size: 40px;">¡${
                              colaborador.nombre
                            }, Comparte los Festejos!</h2>
    ...
    ejecutarFuncionDiaria(
      horaDeseada,
      minutoDeseado,
      enviarCorreoCumpleanieros
    );
  } catch (error) {
